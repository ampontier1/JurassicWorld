package controller;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import model.Dinosaur;

/**
 * Servlet implementation class DirectorServlet
 */
@WebServlet("/directorServlet")
public class DirectorServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DirectorServlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
		
		DinosaurHelper dino = new DinosaurHelper();
		
		String act = request.getParameter("doThisToDino");
		
		String path = "/viewAllDinosServlett";
		
		if (act.equals("delete")) {
			
			try {
				int tempId = Integer.parseInt(request.getParameter("id"));
				Dinosaur dinoToDelete = dino.searchForDinoById(tempId);
				dino.deleteDino(dinoToDelete);
			} catch (NumberFormatException e) {
				System.out.println("Forgot to select an dino");
			}
			
		} else if (act.equals("edit")) {
			try {
				int tempID = Integer.parseInt(request.getParameter("id"));
				Dinosaur dinoToEdit = dino.searchForDinoById(tempID);
				request.setAttribute("dinoToEdit", dinoToEdit);
				path = "/edit-item.jsp";
			} catch (NumberFormatException e) {
				System.out.println("Forgot to select an dino");
			}
			
		} else if (act.equals("add")) {
			path = "/index.html";
		}
		
		getServletContext().getRequestDispatcher(path).forward(request,response);
	}

}
