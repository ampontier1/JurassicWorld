package controller;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import model.Dinosaur;

/**
 * Servlet implementation class EditSurveyServlet
 */
@WebServlet("/editSurveyServlet")
public class EditSurveyServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EditSurveyServlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		DinosaurHelper dino = new DinosaurHelper();
		
		String species = request.getParameter("species");
		String color = request.getParameter("color");
		int tempId = Integer.parseInt(request.getParameter("id"));
		
		Dinosaur dinoToUpdate = dino.searchForDinoById(tempId);
		dinoToUpdate.setColor(color);
		dinoToUpdate.setSpecies(species);
		
		dino.updateDino(dinoToUpdate);
		
		getServletContext().getRequestDispatcher("/viewAllDinosServlet").forward(request, response);
	}

}
